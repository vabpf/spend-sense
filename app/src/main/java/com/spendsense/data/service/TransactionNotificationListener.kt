...
class TransactionNotificationListener : NotificationListenerService() {
    ...
    private suspend fun processNotification(sbn: StatusBarNotification) {
        ...
        if (appPatterns.isEmpty()) {
            ...
            saveToInbox(packageName, notificationText, null)
        ...
        if (!notificationTitle.isNullOrBlank()) {
            ...
            if (matchedPattern != null) {
                ...
                if (matchedPattern.regex != null) {
                    ...
                    saveToInbox(packageName, notificationText, matchedPattern.id)
    ...
    private suspend fun saveToInbox(packageName: String, text: String, stalePatternId: Long?) {
            ...
            if (matchResult != null) {
                ...
                if (amountStr != null && merchant != null) {
                    ...
                    if (amount > 0) {
                        ...
                        saveAndPostNotification(amount, merchant, packageName, appName, pattern.currencyCode, notificationText)
                        return true
                    }
                }
            }
    ...
    private fun postTransactionNotification(
        ...
        val bodyText = if (suggestedCategoryName != null) {
            "$$amount — Suggested: $suggestedCategoryName"
        } else {
            "$$amount — Tap to categorize"
        }

        // Save action
        val ourPkg = this@TransactionNotificationListener.packageName
        val saveIntent = Intent(TransactionNotificationReceiver.ACTION_SAVE).apply {
            setPackage(ourPkg)
            putExtra(TransactionNotificationReceiver.EXTRA_AMOUNT, amount)
            putExtra(TransactionNotificationReceiver.EXTRA_MERCHANT, merchant)
            putExtra(TransactionNotificationReceiver.EXTRA_CURRENCY, currencyCode)
            putExtra(TransactionNotificationReceiver.EXTRA_PACKAGE_NAME, packageName)
            putExtra(TransactionNotificationReceiver.EXTRA_APP_NAME, appName)
            putExtra(TransactionNotificationReceiver.EXTRA_RAW_NOTIFICATION_ID, rawNotificationId)
            if (suggestedCategoryId != null) {
                putExtra(TransactionNotificationReceiver.EXTRA_SUGGESTED_CATEGORY_ID, suggestedCategoryId)
            }
        }
        val savePendingIntent = PendingIntent.getBroadcast(
            this, notificationId, saveIntent,
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
        )

        // Edit action — opens main app with data
        ...
        Log.d(TAG, "Transaction notification posted: $merchant — $amount")
    }
